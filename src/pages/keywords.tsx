...
export default function KeywordsPage({ topics }: { topics: TopicMeta[] }) {
  ...
  topics.forEach(t => (t.keywords || []).forEach(k => {
    const key = String(k);
  ...
  const jpLetters = "あいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほまみむめもやゆよらりるれろわをん".split("");
    ...
    <Layout title="索引">
      ...
      <div className="sticky top-0 z-10 bg-white/70 dark:bg-gray-900/70 backdrop-blur border-y py-2 mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {jpLetters.map(l => <a key={l} href={`#jp-${l}`} className="px-2 py-1 border rounded text-sm">{l}</a>)}
          <a href="#jp-その他" className="px-2 py-1 border rounded text-sm">その他</a>
        </div>
        <div className="text-sm mt-3 mb-1">English</div>
        <div className="flex flex-wrap gap-2">
          {enLetters.map(l => <a key={l} href={`#en-${l}`} className="px-2 py-1 border rounded text-sm">{l}</a>)}
          <a href="#en-Others" className="px-2 py-1 border rounded text-sm">Others</a>
        </div>
      </div>
              ...
              {(jp.get(letter) || []).map(([k, arr]) => (
                <li key={k} className="border rounded p-3">
                  ...
                  <ul className="mt-2 text-sm list-disc pl-5">
                    {arr.map(t => <li key={t.slug}><a className="underline" href={`/topics/${t.slug}`}>{t.title}</a></li>)}
                  </ul>
                </li>
              ))}
              {(jp.get(letter) || []).length === 0 && <li className="opacity-60">（なし）</li>}
            </ul>
          </div>
        ))}
        ...
        {["Others", ...enLetters].map(letter => (
          <div key={`en-${letter}`} id={`en-${letter}`}>
            ...
            <ul className="space-y-2">
              ...
              {(en.get(letter) || []).length === 0 && <li className="opacity-60">（なし）</li>}
            </ul>
          </div>
        ))}
      </section>
