...
export default function Home({ topics, docs }: { topics: TopicMeta[]; docs: Doc[] }) {
  ...
  return (
    <Layout>
      ...
      <h2 className="mt-8 mb-3 font-semibold">タグ</h2>
      <div className="flex flex-wrap gap-2">
        {tags.map(([t, n]) => (
          <a key={t} href={`/tags/${encodeURIComponent(t)}`} className="text-sm px-2 py-1 border rounded">
      ...
      <div className="mt-8">
        <a className="underline" href="/keywords">👉 索引ページへ</a>
      </div>
...
export async function getStaticProps() {
  ...
  const docs = JSON.parse(fs.readFileSync(path.join(dataDir, "search.json"), "utf-8"));
