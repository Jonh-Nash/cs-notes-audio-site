...
export default function TopicPage({ slug, html, meta }: { slug: string; html: string; meta: any }) {
  ...
  return (
    <Layout title={meta.title}>
      ...
      <div className="mb-4">{(meta.tags || []).map((t: string) => <TagBadge key={t} tag={t} />)}</div>

      <div className="mb-6 flex items-center gap-2">
        <button onClick={playTopic} className="border rounded px-3 py-2">🎲 このトピックをランダム再生</button>
      </div>
      ...
      <div className="space-y-4 mb-8">
        {(meta.audio || []).map((src: string, i: number) => (
          <div key={i} className="border rounded p-3">
            ...
            <audio controls preload="none" src={withBase((router as any).basePath || "", src)} className="w-full" />
          </div>
        ))}
      </div>
